...
xmlsecommand = "/opt/xmlsectool/xmlsectool.sh"
cert="/opt/xmlsectool/sign.crt"
certkey="/opt/xmlsectool/sign.key"
cerpass=""
...
def base64_url_decode(inp):
    inp2 = str(inp, 'utf-8')
    padding_factor = (4 - len(inp2) % 4) % 4
    print (padding_factor)
    inp2 += "="*padding_factor
    out = base64.b64decode(inp2.translate(dict(zip(map(ord,
u'-_'), u'+/'))))
    return out
...
def signer(data):
    ...
    if data['type'] in allowedtypes:
        ...
        print('Signing metadata for %r has beend finished' %encnamestr)

channel.basic_qos(prefetch_count=1)
channel.basic_consume('metadatasigner',
                      finalcallback)
